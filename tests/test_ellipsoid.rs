...
fn test_reference() {
    ...
    for (name, ref_val) in models {
        let ell = maprs3d::Ellipsoid::get(name).unwrap();
...
#[test]
fn test_bad_name() {
    let ell = maprs3d::Ellipsoid::get("bad_name");
    assert!(ell.is_none());
}
...
fn test_ellipsoid() {
    ...
    let ell = maprs3d::Ellipsoid::get("maupertuis").unwrap();
